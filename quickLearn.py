...
def mainGame(movementInfo):
    ...
    while True:
        ...
        if bot.actions(-playerx + myPipe["x"], -playery + myPipe["y"], playerVelY):
        ...
        if crashTest[0]:
            # Update the q scores
            bot.writeQval(dump_q_values=False)
...
def showGameOverScreen(crashInfo):
    if VERBOSE:
        ...
        print(str(bot.game_count - 1) + " | " + str(score))

    if bot.game_count == (ITERATIONS):
        bot.dump_q_values(force=True)
        sys.exit()
